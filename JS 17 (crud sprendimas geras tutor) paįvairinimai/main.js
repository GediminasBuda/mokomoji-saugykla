...
const createRow = (numeris, greitis, laikas, data, inicialai) => {
    let tr = `
    <tr>
        <td>${numeris}</td>
        <td>${greitis}</td>
        ...
        <td>${data}</td>
        <td>${inicialai}</td>
        <td><button type="button" class="btn btn-info edit" data-bs-toggle="modal" data-bs-target="#edit-modal">Redaguoti</button></td>
        <td><button type="button" class="btn btn-danger delete" data-bs-toggle="modal" data-bs-target="#delete-modal">Ištrinti</button></td>
        <td><button type="button" class="btn btn-warning warning" data-bs-toggle="modal" data-bs-target="#delete-modal">Testas</button></td>
    </tr>`;
    table.innerHTML += tr;
}
...
data.forEach(item => {
    createRow(item.numeris, item.greitis, item.laikas, item.data, item.inicialai);
});

// Eilutes pridejimas
addForm.addEventListener('submit', e => {
    e.preventDefault();
    createRow(number.value, speed.value, time.value, date.value, inicialai.value);
    addForm.reset();
    newAddModal.hide();
});
table.addEventListener('click', e => {
    ...
    if (e.target.matches('.delete')) {
        ...
        nameEdit.value = editValues[4].innerText;
    }
});
...
editForm.addEventListener('submit', e => {
    ...
    editValues[4].innerText = nameEdit.value;
    newEditModal.hide();
})